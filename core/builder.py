...
def make_dataset() -> Dataset:
    if configs.dataset.name == 'semantic_kitti':
        from core.datasets import SemanticKITTI as SemanticKITTI
...
def make_scheduler(optimizer: Optimizer) -> Scheduler:
    ...
    elif configs.scheduler.name == 'multistep':
        iter_per_epoch = (configs.data.training_size + configs.batch_size - 1) // configs.batch_size
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                         [5*iter_per_epoch,10*iter_per_epoch,15*iter_per_epoch, 25*iter_per_epoch], # milestones is iteration number here
                                                         gamma=0.5,
                                                         last_epoch=-1)
    else:
